...
<script type="text/javascript">
	$("#${id}Button, #${id}Name").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${fns:escapeHtml(url)}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 520, {
				...
				var pid=$("#${id}Id").val();
				 $.ajax({
						...
						success : function(data) {
							...
							$("#townSelect").prepend("<option value=''>--请选择--</option>");
							for (var i=0;i<data.length;i++) {
								 //添加option
								 $("#townSelect").prepend("<option value="+data[i].id+">"+data[i].name+"</option>");
							}
						},
					});
