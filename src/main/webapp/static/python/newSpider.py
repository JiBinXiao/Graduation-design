    ...
    try:
        ...
        for i in range(1, int(length)):
            ...
            for li in lis:
                ...
                sql = "insert into {} (title,prNow,prPre,author,publish,desc1,isbn,review_count,link,type,createdate) values ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(
                    tableName,
                    title,
                    prNow,
                    prPre,
...
def JDCrwal(keyword, num):
    ...
    try:
        ...
        print('\n提示：开始爬取京东页面...')
        for i in range(1, 20):
            url = 'https://search.jd.com/Search?keyword={}&page={}&psort={}'.format(keyword, i , psort)
            soup = getSoupObject(url)
            ...
            for li in lis:
                ...
                sql = "insert into {} (title,prNow,publish,isbn,link,goodRateShow,goodCountStr,createdate,type) " \
                  "values ('{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(tableName, title, prNow, pb, isbn,
                                                                       link ,goodRateShow,goodCountStr, datetime.now(), keyword)
...
def testcrwal(type, keyword, num):
    ...
    if type == 'dangdang':
        print('go dangdang')
        dangDangCrwal(keyword,c)
    closeDB()
    # os.system('pause')


testcrwal(sys.argv[1], sys.argv[2], sys.argv[3])
#testcrwal("jingdong", "c","3")