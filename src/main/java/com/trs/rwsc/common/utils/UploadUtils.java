...
//	@SuppressWarnings("unchecked")
	...
	* @备注：
	 */
	public String[] springUploadFile(MultipartFile file, String savePath, int id) throws IOException {
		//返回数据  ret[0] : 上传文件的原始文件名  ret[1] : 文件保存的完整路径
		...
		if (!file.isEmpty()) {
			...
			//把文件名替换成附件表的id	
			fileName = fileName.replace(name, String.valueOf(id));
			ret[1] = savePath + fileName;
